...
This repository includes Python packaging metadata (`setup.py`) and plugin entry-point registration (`draft_scope.plugin:get_plugin`) so Scope can discover it in plugin pickers after install. It publishes entry points in multiple common Scope/Daydream groups to maximize compatibility across versions.
